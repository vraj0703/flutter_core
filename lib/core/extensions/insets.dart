...
extension InsetsOnIntListX on List<int> {
  Insets get insets => Insets.ofList(this);
}

extension InsetsOnDoubleListX on List<double> {
  Insets get insets => Insets.ofList(this);
}
...
class Insets {
  ...
  factory Insets.ofList(List<num> value) => Insets._(value);

  List<int> get list => value;

  EdgeInsetsDirectional get topBottom => EdgeInsetsDirectional.only(
      top: list[0].toDouble(), bottom: list[1].toDouble());

  EdgeInsetsDirectional get startEnd => EdgeInsetsDirectional.only(
      start: list[0].toDouble(), end: list[1].toDouble());

  EdgeInsets get vertHorz => EdgeInsets.symmetric(
      vertical: list[0].toDouble(), horizontal: list[1].toDouble());

  EdgeInsetsDirectional get steb => EdgeInsetsDirectional.fromSTEB(
      list[0].toDouble(),
      list[1].toDouble(),
      list[2].toDouble(),
      list[3].toDouble());
